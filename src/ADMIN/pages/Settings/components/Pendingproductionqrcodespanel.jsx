...
export default function PendingProductionQrCodesPanel({ open, onClose }) {
  ...
  const fetchPending = () => {
    ...
    Promise.all([
      api.get('/production-qr/qr-codes/pending'),
      ...
      .then(([pendingRes, printersRes, templatesRes]) => {
        ...
        setPrinters(printersRes.data.data ?? printersRes.data);
  ...
  const openPrinterPicker = (record) => {
    ...
    setSelectedPrinterId(record.printer_id ?? null);
    setSelectedTemplateId(record.template_id ?? null);
...
const previewSampleData = printerModalBucket
  ? {
      ...
      product_name: printerModalBucket.product_name,
      part_code: printerModalBucket.part_code,
      erp_no: printerModalBucket.product_erp_no,
    }
  : null;
  ...
  const runReprint = async () => {
    ...
    const bucketId = printerModalBucket.bucket_id;

    const printer = printers.find((p) => p.id === selectedPrinterId);

    if (!printer?.printer_name) {
      message.error(`Printer "${printer?.name || selectedPrinterId}" is missing a printer_name.`);
      return;
    }
    ...
    try {
      ...
      const { data } = await api.post('/production-qr/qr-codes/reprint', {
        bucket_id: bucketId,
      ...
      if (!qrCodes.length) {
        throw new Error('No pending QR codes were returned for reprint');
      }
      ...
      try {
        ...
        console.error('Production QR reprint failed:', printError);

        message.warning(`${qrCodes.length} QR codes are still pending because printing failed.`);
      ...
      if (printedIds.length) {
        await api.post('/production-qr/qr-codes/mark-printed', {
          ids: printedIds,
        });
      }

      message.success(`Reprinted ${qrCodes.length} QR codes`);
      ...
      const clientMessage = !err?.response ? err?.message : null;

      message.error(backendMessage || clientMessage || 'Reprint failed — codes are still saved as pending.');
    } finally {
  ...
  const columns = [
    ...
    {
      title: 'Bucket',
      render: (_, r) => `${String(r.year).padStart(2, '0')}-W${String(r.week).padStart(2, '0')}`,
    },
...

    {
      ...
      render: (_, r) => (
        <Button icon={<PrinterOutlined />} loading={reprintingBucketId === r.bucket_id} onClick={() => openPrinterPicker(r)}>
          Reprint
        </Button>
      ),
    },
  ...
  const reprinting = reprintingBucketId === printerModalBucket?.bucket_id;
  ...
  return (
    <>
      ...
      <Drawer title="Pending QR Codes" open={open} onClose={onClose} size="large">
        <Text type="secondary">
          QR codes that were generated and reserved but never confirmed as printed — usually because a print job
          failed or was interrupted. Reprinting doesn't create new serials.
        </Text>
      ...
      <Modal
        ...
        onCancel={reprinting ? undefined : () => setPrinterModalBucket(null)}
        closable={!reprinting}
        ...
        width={600}
        footer={printProgress ? [] : undefined}
      >
        {printProgress ? (
          <div>
            <Text strong>
            ...
            <Text type="secondary">Print job has been submitted to the selected printer.</Text>
          </div>
        ) : (
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            {selectedTemplate && previewSampleData && (
              <div style={{ marginTop: 12 }}>
                <LabelTemplatePreview template={selectedTemplate} sampleData={previewSampleData} maxWidth={500} />
              </div>
            )}
          </Space>
        )}
